...
export const DetailTopbar: React.FC<DetailTopbarProps> = ({
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("mochi_notifications") || "[]";
  ...
  const handleWatchMovie = (e: React.MouseEvent, movie: MovieCard) => {
    e.stopPropagation();
    setShowDropdown(false);
    if (typeof window !== "undefined" && (window as any).MochiLoader?.showMovie) {
      (window as any).MochiLoader.showMovie(movie.name || movie.slug, movie.source || "kkphim");
  ...
  const handleNotificationClick = () => {
    if (unreadCount > 0) {
      onShowToast?.(`Bạn có ${unreadCount} thông báo mới`);
    } else {
      onShowToast?.("Chưa có thông báo mới nào");
    }
  };
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                      <button
                        ...
                        onClick={(e) => handleWatchMovie(e, item)}
                        title="Xem ngay"
