...
assert(
  spriteStat.size > 1000000,
  `LỖI: Kích thước spritesheet không hợp lệ (${spriteStat.size} bytes)`,
);
console.log(`   ✓ File ảnh spritesheet đầy đủ (${Math.round(spriteStat.size / 1024)} KB).`);

// 2. Kiểm tra mã nguồn MochiLoadingScreen.tsx
console.log(
  "2. Kiểm tra logic điều kiện 'Chỉ khi vào web mới hiển thị còn vào các trang khác không cần'...",
);
...
assert(
  compCode.includes("waving") && compCode.includes("jumping") && compCode.includes("working"),
  "LỖI: Thiếu các animation states của Pet",
);
...
assert(
  compCode.includes('window.addEventListener("pageshow"'),
  "LỖI: Vào lại web không tự chạy loader",
);
assert(compCode.includes("event.persisted"), "LỖI: Loader chưa xử lý trang quay lại từ bfcache");
console.log("   ✓ Logic vào web và quay lại từ bfcache đều tự chạy animation.");
...
assert(
  rootCode.includes("<MochiLoadingScreen />") || rootCode.includes("<MochiLoadingScreen/>"),
  "LỖI: Thiếu thẻ JSX MochiLoadingScreen trong RootComponent",
);
