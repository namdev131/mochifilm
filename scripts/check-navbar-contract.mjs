...
assert(
  code.includes('aria-label="Mở menu"'),
  "LỖI: Thiếu nút bấm mở menu trên mobile (aria-label)",
);
assert(
  code.includes('aria-label="Đóng menu"'),
  "LỖI: Thiếu nút bấm đóng menu trên mobile (aria-label)",
);
assert(
  code.includes("setIsSidebarOpenMobile(false)"),
  "LỖI: Thiếu hành vi tự đóng menu khi chọn mục hoặc bấm backdrop",
);
...
assert(
  code.includes("confirmDeleteFavorite"),
  "LỖI: Thiếu hàm xác nhận trước khi xóa phim yêu thích",
);
assert(
  code.includes("confirmClearAllFavorites"),
  "LỖI: Thiếu hàm xác nhận trước khi xóa tất cả phim yêu thích",
);
assert(
  code.includes("confirmDeleteHistoryItem"),
  "LỖI: Thiếu hàm xác nhận trước khi xóa phim trong lịch sử",
);
assert(
  code.includes("confirmClearAllHistory"),
  "LỖI: Thiếu hàm xác nhận trước khi xóa toàn bộ lịch sử",
);
...
assert(
  code.includes("setShowSourceModal(true)"),
  "LỖI: Nút nguồn phim phải mở modal cấu hình nguồn",
);
...
assert(
  code.includes("fetchCategoryMoviesFromApi"),
  "LỖI: Thiếu hàm fetchCategoryMoviesFromApi gọi API thật",
);
...
assert(
  code.includes("countryQueryMovies"),
  "LỖI: Thiếu live query cho quốc gia countryQueryMovies",
);
...
assert(
  code.includes("currentNavMeta.title"),
  "LỖI: Heading chuyên mục phải hiển thị title chính xác",
);
...
assert(
  code.includes('selectedNav === "the-loai"'),
  "LỖI: Thể loại phải kích hoạt active state 'the-loai'",
);
assert(
  code.includes('selectedNav === "quoc-gia"'),
  "LỖI: Quốc gia phải kích hoạt active state 'quoc-gia'",
);
...
assert(
  !code.includes('<nav aria-label="Breadcrumb"'),
  "LỖI: Không được tạo thêm navbar phụ hay breadcrumbs nav!",
);
...
console.log(
  "    ✓ Giữ nguyên đúng 1 sidebar/navbar hiện tại (268px), không có navbar phụ hay giao diện khác.",
);
...
assert(
  code.includes('setSelectedGenre("Tất cả")'),
  "LỖI: Trang chủ phải reset thể loại về 'Tất cả'",
);
assert(code.includes("setSelectedCountry(null)"), "LỖI: Trang chủ phải reset quốc gia về null");
assert(
  code.includes("sortByNewest"),
  "LỖI: Thiếu hàm sắp xếp phim mới nhất sortByNewest trong index.tsx",
);
assert(apiCode.includes("sortByNewest"), "LỖI: Thiếu hàm sortByNewest trong api.ts");
console.log(
  "    ✓ Trang chủ reset toàn bộ bộ lọc (nav, genre, country, search). Phim mới sắp xếp mới nhất chuẩn thời gian modified/year.",
);
...
assert(
  code.includes('m.type === "series"'),
  "LỖI: Phim bộ phải lọc dựa trên metadata type === 'series'",
);
assert(
  code.includes('m.type === "single"'),
  "LỖI: Phim lẻ phải lọc dựa trên metadata type === 'single'",
);
assert(
  code.includes('m.type === "hoathinh"'),
  "LỖI: Hoạt hình phải lọc dựa trên metadata type === 'hoathinh'",
);
assert(
  code.includes("m.chieu_rap === true"),
  "LỖI: Phim chiếu rạp phải lọc dựa trên metadata chieu_rap === true",
);
...
console.log(
  "    ✓ Phim lẻ, phim bộ, hoạt hình, chiếu rạp lọc chính xác theo metadata đã normalize (type, category, status, chieu_rap).",
);

// 13. Kiểm tra Đổi navbar: đóng mobile/dropdown, cuộn main lên đầu, đồng bộ URL query & back-forward (popstate)
console.log(
  "13. Kiểm tra Đổi navbar: đóng mobile/dropdown, cuộn main lên đầu, đồng bộ URL query & back-forward...",
);
assert(
  code.includes("setShowGenreDropdown(false)"),
  "LỖI: Phải đóng genre dropdown khi đổi navbar",
);
assert(
  code.includes("setShowCountryDropdown(false)"),
  "LỖI: Phải đóng country dropdown khi đổi navbar",
);
assert(
  code.includes("setShowSearchDropdown(false)"),
  "LỖI: Phải đóng search dropdown khi đổi navbar",
);
assert(code.includes("window.scrollTo({ top: 0"), "LỖI: Phải cuộn trang lên đầu khi đổi navbar");
assert(
  code.includes("window.history.pushState"),
  "LỖI: Phải đồng bộ URL query qua history.pushState khi đổi navbar",
);
assert(
  code.includes('window.addEventListener("popstate"'),
  "LỖI: Phải lắng nghe popstate để hỗ trợ back/forward",
);
console.log(
  "    ✓ Đổi navbar tự đóng mobile menu & dropdown, cuộn main lên đầu, đồng bộ URL query và hỗ trợ refresh/back-forward.",
);

// 14. Kiểm tra Bộ lọc Nguồn áp dụng đồng bộ với mọi mục
console.log("14. Kiểm tra Bộ lọc Nguồn áp dụng đồng bộ với mọi mục...");
assert(
  code.includes("handleSelectSource"),
  "LỖI: Thiếu hàm handleSelectSource đồng bộ bộ lọc Nguồn",
);
assert(code.includes("displayedFavorites"), "LỖI: Thiếu displayedFavorites đồng bộ theo Nguồn");
assert(
  code.includes("displayedContinueList"),
  "LỖI: Thiếu displayedContinueList đồng bộ theo Nguồn",
);
assert(
  code.includes('params.set("source", newSource)'),
  "LỖI: Nguồn phải được đồng bộ vào URL query params",
);
assert(
  code.includes('params.set("source", selectedSource)'),
  "LỖI: Điều hướng chuyên mục phải bảo lưu nguồn đang chọn",
);
assert(
  code.includes("favorites.filter((f) => f.source === selectedSource)"),
  "LỖI: Yêu thích phải lọc theo selectedSource",
);
assert(
  code.includes("continueList.filter((item) => item.source === selectedSource)"),
  "LỖI: Lịch sử phải lọc theo selectedSource",
);
assert(
  code.includes('queryKey: ["latestMovies", selectedSource]'),
  "LỖI: Phim mới/Trang chủ phải đồng bộ theo selectedSource",
);
assert(
  code.includes('queryKey: ["genreQuery", selectedGenre, selectedSource]'),
  "LỖI: Thể loại phải đồng bộ theo selectedSource",
);
assert(
  code.includes('queryKey: ["countryQuery", selectedCountry, selectedSource]'),
  "LỖI: Quốc gia phải đồng bộ theo selectedSource",
);
assert(
  code.includes(
    'queryKey: ["navCategoryQuery", selectedNav, navCategoryConfig?.slug, selectedSource]',
  ),
  "LỖI: Chuyên mục phải đồng bộ theo selectedSource",
);
console.log(
  "    ✓ Bộ lọc Nguồn áp dụng đồng bộ 100% trên toàn bộ các mục (Trang chủ, Phim mới, Phim lẻ, Phim bộ, Chiếu rạp, Hoạt hình, Thể loại, Quốc gia, Yêu thích, Lịch sử, Tìm kiếm).",
);
...
assert(
  code.includes("showVipNoticeModal"),
  "LỖI: Mục VIP phải có modal giải thích chi tiết gói VIP",
);
assert(
  code.includes("setShowVipNoticeModal(true)"),
  "LỖI: Nút Chi tiết tính năng VIP phải kích hoạt modal",
);
assert(
  code.includes("userMenuRef"),
  "LỖI: Avatar người dùng phải có userMenuRef và menu chức năng",
);
assert(code.includes("showUserMenu"), "LỖI: Thiếu state showUserMenu trên Avatar");
assert(
  code.includes("notificationDropdownRef"),
  "LỖI: Chuông thông báo phải có notificationDropdownRef",
);
assert(code.includes("mascot-chair.webp"), "LỖI: Sidebar phải có Mascot Ghế chuẩn");
console.log(
  "    ✓ Đầy đủ 100% chức năng cho toàn bộ các mục: Trang chủ, Phim mới, Phim lẻ, Phim bộ, Chiếu rạp, Hoạt hình, 15 Thể loại, 10 Quốc gia, Yêu thích, Lịch sử, Nguồn phim, Mochi VIP, User Avatar Menu.",
);

// 16. Kiểm tra Thể loại và Quốc gia: dropdown, lọc thật, Đặt lại phục hồi, click ngoài, Escape, ARIA, focus/keyboard
console.log(
  "16. Kiểm tra Thể loại và Quốc gia (dropdown, aria-expanded, aria-controls, focus/keyboard, Đặt lại)...",
);
...
assert(
  code.includes("genreButtonRef.current?.focus()"),
  "LỖI: Khi bấm Escape phải hoàn trả focus về genre trigger button",
);
assert(
  code.includes("countryButtonRef.current?.focus()"),
  "LỖI: Khi bấm Escape phải hoàn trả focus về country trigger button",
);
...
assert(
  code.includes('aria-label="Đặt lại thể loại, phục hồi danh sách"'),
  "LỖI: Thiếu nút Đặt lại phục hồi danh sách trong Thể loại",
);
assert(
  code.includes('aria-label="Đặt lại quốc gia, phục hồi danh sách"'),
  "LỖI: Thiếu nút Đặt lại phục hồi danh sách trong Quốc gia",
);
assert(
  code.includes("fetchCategoryMoviesFromApi"),
  "LỖI: Phải gọi API nạp dữ liệu thật khi chọn thể loại / quốc gia",
);
console.log(
  "    ✓ Thể loại và Quốc gia đầy đủ mở/đóng, lọc thật, nút Đặt lại phục hồi danh sách, click ngoài & Escape đóng, aria-expanded/aria-controls, focus/keyboard chuẩn WAI-ARIA.",
);
...
assert(
  code.includes("validateSearch"),
  "LỖI: Route phải có validateSearch để TanStack Router đồng bộ URL search params",
);
assert(
  code.includes("Route.useSearch()"),
  "LỖI: Component phải đọc search params từ Route.useSearch()",
);
assert(
  code.includes("Route.useNavigate()"),
  "LỖI: Component phải dùng navigate từ TanStack Router để cập nhật URL",
);
assert(
  code.includes('params.get("q")'),
  "LỖI: syncStateFromUrl phải đọc tham số tìm kiếm 'q' từ URL",
);
assert(
  code.includes("searchParams.q"),
  "LỖI: Component phải đồng bộ tham số tìm kiếm q từ searchParams",
);
console.log(
  "    ✓ Đồng bộ URL query parameters và tham số tìm kiếm q hoạt động chuẩn xác với TanStack Router.",
);
