...
assert(watchRouteCode.includes('createFileRoute("/watch/$slug")'), "LỖI: Route phải đăng ký createFileRoute('/watch/$slug')");
assert(watchRouteCode.includes("validateSearch"), "LỖI: Route phải có validateSearch kiểm tra source, ep, srv");
assert(watchRouteCode.includes("Route.useParams()"), "LỖI: Route phải đọc slug qua Route.useParams()");
assert(watchRouteCode.includes("Route.useSearch()"), "LỖI: Route phải đọc search params qua Route.useSearch()");
...
assert(routeTreeCode.includes("/watch/$slug"), "LỖI: routeTree.gen.ts chưa đăng ký route /watch/$slug");
...
assert(watchRouteCode.includes("lv-favorites"), "LỖI: Phải đồng bộ danh sách yêu thích vào lv-favorites");
assert(!watchRouteCode.includes("position: 60") && !watchRouteCode.includes("duration: 2700"), "LỖI: Cấm ghi đè cứng tiến độ position:60, duration:2700!");
assert(watchRouteCode.includes("handleTimeProgress"), "LỖI: watch.$slug.tsx phải nhận callback handleTimeProgress");
...
assert(controlsCodeCheck.includes("handleLoadedMetadata"), "LỖI: PlayerControls phải khôi phục vị trí lưu qua handleLoadedMetadata");
assert(controlsCodeCheck.includes("onTimeProgress"), "LỖI: PlayerControls phải gửi callback onTimeProgress về route");
...
assert(panelsCode.includes("mochi-comments-"), "LỖI: Phải lưu bình luận thật theo từng phim trong localStorage");
console.log("   ✓ Real-data wiring hoàn chỉnh: API thật, TanStack Query, HTMLVideoElement telemetry, localStorage progress/favorites/comments.");
...
assert(topbarCode.includes('aria-label="Tìm kiếm"'), "LỖI: Topbar thiếu aria-label cho ô/nút tìm kiếm");
assert(topbarCode.includes('aria-label="Thông báo"'), "LỖI: Topbar thiếu aria-label cho thông báo");
assert(!topbarCode.includes('aria-label="Đổi nguồn phim"'), "LỖI: Topbar không được có nút đổi nguồn");
...
const controlsChromeCode = readFileSync(join(projectRoot, "src/components/player/PlayerControlsChrome.tsx"), "utf8");
const controlsUiCode = `${controlsCode}\n${controlsChromeCode}`;
assert(controlsUiCode.includes('aria-label="Phát"') || controlsUiCode.includes('aria-label={isPlaying ? "Tạm dừng" : "Phát"}'), "LỖI: PlayerControls thiếu aria-label phát/tạm dừng");
assert(controlsUiCode.includes('aria-label="Thanh âm lượng"'), "LỖI: PlayerControls thiếu aria-label cho thanh âm lượng");
assert(controlsUiCode.includes('aria-label="Tốc độ phát"'), "LỖI: PlayerControls thiếu aria-label cho tốc độ phát");
assert(controlsUiCode.includes('aria-label="Toàn màn hình"'), "LỖI: PlayerControls thiếu aria-label toàn màn hình");
assert(controlsUiCode.includes('aria-label="Chế độ rạp"'), "LỖI: PlayerControls thiếu aria-label chế độ rạp");
assert(controlsCode.includes('aria-label="Báo lỗi nguồn phát"'), "LỖI: PlayerControls thiếu aria-label báo lỗi");

assert(panelsCode.includes('aria-label="Nội dung bình luận"'), "LỖI: PlayerPanels thiếu aria-label cho ô bình luận");
assert(panelsCode.includes('aria-label="Đăng bình luận"'), "LỖI: PlayerPanels thiếu aria-label cho nút đăng bình luận");
console.log("   ✓ Đầy đủ các hook lifecycle và toàn bộ WAI-ARIA labels trợ năng chính.");

const rightbarCode = readFileSync(join(projectRoot, "src/components/player/PlayerRightbar.tsx"), "utf8");
const playerTopbarCode = readFileSync(join(projectRoot, "src/components/player/PlayerTopbar.tsx"), "utf8");
for (const [name, source] of [["route", watchRouteCode], ["rightbar", rightbarCode], ["topbar", playerTopbarCode]]) {
  for (const removedSourceUi of ["sourceList", "apiCode", "source-pill", "Nguồn phát", "API player đang dùng", "onSelectSource", "onOpenSourceList"]) {
    assert(!source.includes(removedSourceUi), `LỖI: ${name} không được còn UI nguồn phát: ${removedSourceUi}`);
  }
}
...
const checkedSource = checkedFiles.map((relPath) => {
...
console.log("   ✓ Không có fallback demo/mock hoặc số tiến độ dựng sẵn; contract tự chứng minh bắt được regression.");
...
assert(watchRouteCode.includes("onTimeProgress={handleTimeProgress}"), "LỖI: Route chưa nhận telemetry thật từ player");
assert(watchRouteCode.includes("persistProgress(currSec, totalSec)"), "LỖI: Route chưa lưu currentTime/duration thật");
...
assert(!controlsCode.includes('!activeStreamUrl.includes(".m3u8") ? activeStreamUrl : null'), "LỖI: Cấm coi mọi URL không chứa .m3u8 là iframe");
assert(!/isEmbed\s*=\s*Boolean\(\s*activeStreamUrl\s*\)/.test(controlsCode), "LỖI: Cấm coi mọi URL HTTP là iframe");
...
assert(isDirectVideo("https://cdn.example.com/video.mp4?token=123"), "LỖI: URL .mp4 phải là video trực tiếp");
assert(!isDirectVideo("https://cdn.example.com/stream.m3u8"), "LỖI: .m3u8 không được nhận là MP4");
console.log("   ✓ HLS dùng hls.js/native HLS, video trực tiếp dùng <video>, iframe chỉ dành cho embed thật.");
