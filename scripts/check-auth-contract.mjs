...
assert(
  authCode.includes('createFileRoute("/auth")'),
  "LỖI: Route phải đăng ký createFileRoute('/auth')",
);
assert(
  authCode.includes("validateSearch"),
  "LỖI: Route phải kiểm tra search params mode và redirect",
);
...
assert(
  existsSync(logoPath),
  "LỖI: File logo chuẩn public/assets/mochi/wordmark.webp không tồn tại",
);
...
assert(
  existsSync(mascotPath),
  "LỖI: File mascot public/assets/mochi/mascot-auth.png không tồn tại",
);
assert(
  authCode.includes("/assets/mochi/mascot-auth.png"),
  "LỖI: Route auth phải sử dụng mascot-auth.png",
);
...
assert(
  ...
  "LỖI: Thiếu responsive breakpoint 820px (mobile layout)",
);
assert(
  cssCode.includes("@media (max-width: 560px)"),
  "LỖI: Thiếu responsive breakpoint 560px (small mobile layout)",
);
...
assert(
  authCode.includes("supabase.auth.signInWithPassword"),
  "LỖI: Phải gọi supabase.auth.signInWithPassword",
);
...
assert(
  authCode.includes("mochi:user-changed"),
  "LỖI: Phải phát event mochi:user-changed khi đăng nhập",
);
...
const detailTopbarCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailTopbar.tsx"),
  "utf8",
);
assert(
  detailTopbarCode.includes('to: "/auth"'),
  "LỖI: DetailTopbar phải có liên kết điều hướng to: '/auth'",
);
...
assert(
  playerTopbarCode.includes('to: "/auth"'),
  "LỖI: PlayerTopbar phải có liên kết điều hướng to: '/auth'",
);
