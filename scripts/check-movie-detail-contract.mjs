...
assert(
  routeCode.includes('createFileRoute("/movie/$slug")'),
  "LỖI: Route phải đăng ký createFileRoute('/movie/$slug')",
);
assert(
  routeCode.includes("validateSearch"),
  "LỖI: Route phải có validateSearch để kiểm tra source param",
);
...
assert(
  routeCode.includes("fetchDetail"),
  "LỖI: Phải gọi fetchDetail để lấy thông tin phim từ máy chủ nguồn",
);
assert(
  routeCode.includes("fetchLatest"),
  "LỖI: Phải gọi fetchLatest để lấy danh sách phim đề xuất từ nguồn",
);
assert(
  routeCode.includes("useQuery"),
  "LỖI: Phải dùng TanStack Query để quản lý caching và lifecycle",
);
...
assert(
  homeCode.includes('to: "/movie/$slug"'),
  "LỖI: Trang chủ phải có điều hướng to: '/movie/$slug'",
);
assert(
  homeCode.includes("openMovieDetail"),
  "LỖI: Trang chủ phải có hàm openMovieDetail chuẩn hóa điều hướng",
);
assert(!homeCode.includes("spkỳ phim"), "LỖI: Vẫn còn vết prompt lỗi trong index.tsx");
console.log(
  "   ✓ Thao tác nhấn poster/card trên trang chủ điều hướng trực tiếp sang route chi tiết.",
);
...
const heroCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailHero.tsx"),
  "utf8",
);
assert(heroCode.includes('id="watchNow"'), "LỖI: Hero thiếu nút Xem ngay #watchNow");
assert(
  heroCode.includes('to="/watch/$slug"') || heroCode.includes("onWatchNow"),
  "LỖI: Hero nút Xem ngay phải điều hướng tới /watch/$slug",
);
assert(
  routeCode.includes('to: "/watch/$slug"') && routeCode.includes("navigate"),
  "LỖI: movie.$slug.tsx phải có hàm điều hướng navigate tới /watch/$slug với source/ep/srv",
);
...
const sidebarCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailSidebar.tsx"),
  "utf8",
);
...
assert(
  !routeCode.includes("DetailSourcePanel"),
  "LỖI: Trang chi tiết không được render bảng Nguồn phát bên phải",
);
assert(
  !routeCode.includes("handleOpenSourcePanel"),
  "LỖI: Không được giữ handler mở bảng Nguồn phát đã bỏ",
);
const topbarCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailTopbar.tsx"),
  "utf8",
);
assert(
  !topbarCode.includes("source-pill"),
  "LỖI: Không được giữ nút mở bảng Nguồn phát trên header/mobile",
);
assert(!topbarCode.includes("Đổi nguồn phim"), "LỖI: Không được giữ điều khiển Nguồn phát đã bỏ");
assert(
  !topbarCode.includes("onOpenSourcePanel"),
  "LỖI: Không được giữ callback panel Nguồn phát đã bỏ",
);

const mobileNavCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailMobileNav.tsx"),
  "utf8",
);
...
assert(
  cssCode.includes(".details-root"),
  "LỖI: Thiếu class bao bọc .details-root trong styles/details.css",
);
assert(cssCode.includes("--sidebar"), "LỖI: Thiếu biến layout --sidebar");
assert(
  !cssCode.includes("--source"),
  "LỖI: Không được giữ khoảng trống layout cho bảng Nguồn phát đã bỏ",
);
assert(
  cssCode.includes(".details-root .mascot-balloon"),
  "LỖI: Thiếu rule tỉ lệ cho mascot bóng bay",
);
assert(
  cssCode.includes("object-fit: contain"),
  "LỖI: Mascot phải giữ đúng tỉ lệ ảnh bằng object-fit: contain",
);
assert(
  cssCode.includes("@media (max-width: 860px)"),
  "LỖI: Thiếu responsive breakpoint mobile <= 860px",
);
...
const contentCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailContent.tsx"),
  "utf8",
);
assert(contentCode.includes("player-wrap"), "LỖI: Thiếu khung player phát video");
assert(
  contentCode.includes("episodesPanel") || contentCode.includes("ep-btn"),
  "LỖI: Thiếu danh sách tập phim",
);
...
assert(
  !existsSync(join(projectRoot, "reference_details.html")),
  "LỖI: Vẫn còn file tạm reference_details.html trong repo",
);
assert(
  !existsSync(join(projectRoot, "scratch_details_clean.html")),
  "LỖI: Vẫn còn file tạm scratch_details_clean.html trong repo",
);
assert(
  !existsSync(join(projectRoot, "scripts/clean_ref.mjs")),
  "LỖI: Vẫn còn file tạm clean_ref.mjs trong repo",
);
...
const castCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailCast.tsx"),
  "utf8",
);
assert(
  !castCode.includes("Matthew McConaughey"),
  "LỖI: Vẫn còn mock actor Matthew McConaughey trong DetailCast.tsx",
);
assert(
  !castCode.includes("Anne Hathaway"),
  "LỖI: Vẫn còn mock actor Anne Hathaway trong DetailCast.tsx",
);
...
const commentsCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailComments.tsx"),
  "utf8",
);
assert(
  !commentsCode.includes("MinhKhoa"),
  "LỖI: Vẫn còn mock comment MinhKhoa trong DetailComments.tsx",
);
assert(
  !commentsCode.includes("HoangNam"),
  "LỖI: Vẫn còn mock comment HoangNam trong DetailComments.tsx",
);
assert(
  !commentsCode.includes("pravatar.cc"),
  "LỖI: Vẫn còn pravatar giả lập trong DetailComments.tsx",
);

const recsCode = readFileSync(
  join(projectRoot, "src/components/movie-detail/DetailRecommendations.tsx"),
  "utf8",
);
assert(
  !recsCode.includes("defaultRecs"),
  "LỖI: Vẫn còn defaultRecs trong DetailRecommendations.tsx",
);
assert(
  !recsCode.includes("Inception"),
  "LỖI: Vẫn còn mock Inception trong DetailRecommendations.tsx",
);
assert(
  !recsCode.includes("unsplash.com"),
  "LỖI: Vẫn còn ảnh mock Unsplash trong DetailRecommendations.tsx",
);
assert(
  !recsCode.includes(': "8.5"'),
  "LỖI: Vẫn còn bịa rating fallback '8.5' trong DetailRecommendations.tsx",
);
...
assert(
  ruleContent.includes("Zero Mock Data Policy") || ruleContent.includes("không sử dụng mock data"),
  "LỖI: Nội dung rule chưa nêu rõ chính sách không mock data",
);
