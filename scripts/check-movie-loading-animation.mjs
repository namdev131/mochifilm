...
const compCode = readFileSync(
  join(projectRoot, "src/components/common/MochiLoadingScreen.tsx"),
  "utf8",
);
...
assert(
  !movieRouteCode.includes("Đang nạp dữ liệu chi tiết phim từ máy chủ {"),
  "LỖI: Vẫn còn dòng text thô 'Đang nạp dữ liệu chi tiết phim từ máy chủ...'",
);
assert(
  movieRouteCode.includes("movie-loading-stage"),
  "LỖI: Thiếu stage container chuẩn movie-loading-stage",
);
console.log("   ✓ movie.$slug.tsx đã tích hợp MochiLoader và loại bỏ hoàn toàn text nạp thô.");

// 3. Kiểm tra watch.$slug.tsx
console.log(
  "3. Kiểm tra watch.$slug.tsx tích hợp MochiLoader và thay thế text nạp luồng máy chủ...",
);
...
assert(
  !watchRouteCode.includes("Đang nạp trình phát và luồng video từ máy chủ {"),
  "LỖI: Vẫn còn dòng text thô 'Đang nạp trình phát và luồng video...'",
);
assert(
  watchRouteCode.includes("player-loading-stage"),
  "LỖI: Thiếu stage container chuẩn player-loading-stage",
);
...
assert(
  homeCode.includes("MochiLoader") && homeCode.includes("showMovie"),
  "LỖI: openMovieDetail chưa kích hoạt MochiLoader.showMovie khi nhấn vào phim",
);
...
const detailTopCode = readFileSync(
...
const playerTopCode = readFileSync(
