...
const hook = readFileSync(
  join(root, "src/components/player/hooks/usePlayerControlsVisibility.ts"),
  "utf8",
);
...
assert(
  source.includes("usePlayerControlsVisibility"),
  "PlayerControls phải dùng hook visibility riêng",
);
...
assert(
  hook.includes('matchMedia("(hover: none), (pointer: coarse)")'),
  "Thiếu phát hiện thiết bị cảm ứng",
);
...
assert(
  chrome.includes('aria-label={controlsLocked ? "Mở khóa công cụ" : "Khóa công cụ"}'),
  "Thiếu nút khóa có nhãn truy cập",
);
...
assert(
  ...
  "Thiếu hành vi hover riêng cho desktop",
);
assert(css.includes("@media (hover: none), (pointer: coarse)"), "Thiếu layout touch riêng");
assert(
  chrome.indexOf("<PlayerProgress") < chrome.indexOf('className="player-mobile-primary-controls"'),
  "Progress phải nằm trên hàng Play/±10",
);
assert(
  css.includes(".player-page-root .player-mobile-primary-controls"),
  "Thiếu selector .player-mobile-primary-controls",
);
assert(
  css.includes("top: 50%") && css.includes("left: 50%"),
  "Cụm Play/±10 phải nằm chính giữa video player trên mobile",
);
assert(
  css.includes(".center-play") && css.includes("display: none"),
  "Mobile phải ẩn nút centerPlay trùng lặp",
);
assert(
  css.includes("width: 40px") && css.includes("height: 40px"),
  "Nút tua mobile phải nhỏ gọn, vẫn đủ vùng chạm",
);
assert(
  css.includes("width: 52px") && css.includes("height: 52px"),
  "Nút Play mobile phải chuẩn kích thước 52px",
);
assert(
  source.includes('document.addEventListener("fullscreenchange"'),
  "Thiếu listener fullscreenchange",
);
assert(
  source.includes('orientation.lock?.("landscape")'),
  "Mobile fullscreen phải yêu cầu xoay ngang",
);
assert(source.includes("orientation.unlock?.()"), "Thoát fullscreen phải mở khóa xoay màn hình");
assert(
  chrome.includes("onToggleSubtitles") && chrome.includes("onToggleSettings"),
  "Nút phụ đề/settings phải có handler thật",
);
...
assert(
  !readFileSync(join(root, "src/styles/player.css"), "utf8").includes(".player-controls-lock"),
  "CSS controls còn dồn trong player.css",
);
