...
assert(
  !code.includes("INITIAL_CONTINUE_WATCHING"),
  "LỖI: Vẫn còn INITIAL_CONTINUE_WATCHING trong index.tsx",
);
...
assert(
  code.includes('useState<SourceFilter>("all")'),
  "LỖI: selectedSource chưa khởi tạo mặc định là 'all'",
);
...
assert(
  code.includes("60_000") || code.includes("60000"),
  "LỖI: Thiếu chu kỳ polling 60 giây (60000ms)",
);
...
assert(
  code.includes("removeChannel") || code.includes("unsubscribe"),
  "LỖI: Thiếu cleanup removeChannel/unsubscribe",
);
...
assert(
  !code.includes("vote_average ||"),
  "LỖI: Vẫn còn fallback bịa rating 'vote_average ||' trong index.tsx",
);
...
assert(
  code.includes("vote_average !== undefined"),
  "LỖI: Phải kiểm tra vote_average !== undefined trước khi hiển thị badge rating",
);
...
assert(
  code.includes('table: "notifications"'),
  "LỖI: Chưa đăng ký realtime trên bảng notifications",
);
...
assert(
  code.includes("Chưa đăng nhập") && code.includes("Không có thông báo nào"),
  "LỖI: Thiếu trạng thái rỗng chuẩn",
);
console.log(
  "   ✓ Thông báo realtime Supabase chuẩn: tải thật theo user_id, subscribe realtime, cleanup đầy đủ, không dùng thông báo mẫu.",
);
...
console.log(
  "   ✓ TanStack Query cấu hình chuẩn: refetchInterval 60s, refetchOnWindowFocus, tự refresh giữ source/filter.",
);
...
assert(
  code.includes("Không tìm thấy phim phù hợp"),
  "LỖI: Thiếu trạng thái rỗng khi không có phim",
);
console.log(
  "   ✓ Giao diện responsive hoàn chỉnh: sidebar 268px, loading skeleton, error retry, empty state.",
);
...
assert(
  existsSync(new URL("../public/assets/mochi/wordmark.webp", import.meta.url)),
  "LỖI: Thiếu file wordmark.webp",
);
assert(
  existsSync(new URL("../public/assets/mochi/mascot-mini.png", import.meta.url)),
  "LỖI: Thiếu file mascot-mini.png",
);
assert(
  existsSync(new URL("../public/assets/mochi/mascot-chair.webp", import.meta.url)),
  "LỖI: Thiếu file mascot-chair.webp",
);
assert(
  existsSync(new URL("../public/assets/mochi/mascot-balloon.webp", import.meta.url)),
  "LỖI: Thiếu file mascot-balloon.webp",
);
assert(
  !code.includes("data:image/png;base64,iVBORw0KGgo"),
  "LỖI: Không được dán base64 khổng lồ vào index.tsx",
);
assert(
  !stylesCode.includes("data:image/png;base64,iVBORw0KGgo"),
  "LỖI: Không được dán base64 khổng lồ vào styles.css",
);
...
console.log(
  "   ✓ 4 tài nguyên ảnh thật đã lưu trong public/assets/mochi/; không có base64 khổng lồ; avatar topbar 42x42 chuẩn.",
);
...
assert(
  code.includes("175px") && code.includes("100px"),
  "LỖI: Thiếu kích thước mascot-balloon 175px desktop / 100px mobile",
);
assert(code.includes("mascot-chair.webp"), "LỖI: Thiếu mascot-chair.webp phía trên Premium panel");
assert(
  code.includes("220px") && code.includes("110px"),
  "LỖI: Thiếu kích thước mascot-chair 220px desktop / 110px tablet",
);
assert(stylesCode.includes("@keyframes floaty"), "LỖI: Thiếu animation floaty trong styles.css");
assert(
  stylesCode.includes("prefers-reduced-motion"),
  "LỖI: Thiếu kiểm tra prefers-reduced-motion trong styles.css",
);
assert(code.includes("motion-reduce:animate-none"), "LỖI: Thiếu class motion-reduce:animate-none");
console.log(
  "   ✓ Mascot Balloon Hero (175px/100px) và Mascot Chair (220px/110px) floaty animation hỗ trợ prefers-reduced-motion chuẩn.",
);
